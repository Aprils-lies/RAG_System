...
import com.april.rag.mapper.DocumentVectorMapper;
...
import com.april.rag.service.ElasticsearchService;
...
public class DocumentServiceImpl implements DocumentService {
    ...
    public void deleteDocument(String fileMd5, String userId) {
        ...
        try {
            // 获取文件信息以获取文件名
            FileUpload fileUpload = fileUploadMapper.selectByFileMd5AndUserId(fileMd5, userId);
            if (fileUpload != null) {
    ...
    public List<FileUpload> getAccessibleFiles(String userId, String orgTags) {
        ...
        try {
            ...
            if (userEffectiveTags.isEmpty()) {
                ...
                files = fileUploadMapper.findByUserIdOrIsPublicTrue(userId);
                logger.debug("用户无组织标签，仅返回个人和公开文件");
            } else {
                // 查询用户可访问的所有文件（考虑层级标签）
                files = fileUploadMapper.findAccessibleFilesWithTags(userId, userEffectiveTags);
