...
@Service
public class OrgTagCacheServiceImpl implements OrgTagCacheService {

    private static final Logger logger = LoggerFactory.getLogger(OrgTagCacheServiceImpl.class);
    ...
    @Autowired
    private OrganizationTagMapper orgTagMapper;
    ...
    public List<String> getUserEffectiveOrgTags(String username) {
        try {
            ...
            if (cachedTags != null && !cachedTags.isEmpty()) {
                List<String> effectiveOrgTags = cachedTags.stream()
                        .map(Object::toString)
                        .collect(Collectors.toList());
                // 确保默认标签在结果中（从缓存读取的情况）
                if (!effectiveOrgTags.contains(DEFAULT_ORG_TAG)) {
                    effectiveOrgTags.add(DEFAULT_ORG_TAG);
                }
                return effectiveOrgTags;
            }
    ...
    private void collectParentTags(String tagId, Set<String> result) {
        try {
            OrganizationTag tag = orgTagMapper.findByTagId(tagId);
