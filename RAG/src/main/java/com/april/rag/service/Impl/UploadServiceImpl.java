...
import com.april.rag.mapper.ChunkInfoMapper;
import com.april.rag.mapper.FileUploadMapper;
import com.april.rag.service.UploadService;
...
public class UploadServiceImpl implements UploadService {
    ...
    public void uploadChunk(String fileMd5, int chunkIndex, long totalSize, String fileName, MultipartFile file, String orgTag, boolean isPublic, String userId) throws IOException {
        ...
        try {
            ...
            LambdaQueryWrapper<FileUpload> eq = new LambdaQueryWrapper<FileUpload>()
                    .eq(FileUpload::getFileMd5, fileMd5)
                    .eq(FileUpload::getUserId, userId);

            boolean fileExists = fileUploadMapper.exists(eq);
    ...
    public int getTotalChunks(String fileMd5, String userId) {
        ...
        try {
            LambdaQueryWrapper queryWrapper = new LambdaQueryWrapper<FileUpload>()
                    .eq(FileUpload::getFileMd5, fileMd5)
                    .eq(FileUpload::getUserId, userId)
                    .last("LIMIT 1");
    ...
    public String mergeChunks(String fileMd5, String fileName, String userId) {
        ...
        try {
            ...
            try {
                ...
                LambdaUpdateWrapper<FileUpload> updateWrapper = new LambdaUpdateWrapper<FileUpload>()
                        .eq(FileUpload::getFileMd5, fileMd5)
                        .eq(FileUpload::getUserId, userId)
                        .set(FileUpload::getStatus, 1)
                        .set(FileUpload::getMergedAt, LocalDateTime.now());


                int rows = fileUploadMapper.update(null, updateWrapper);
                if (rows == 0) {
                    logger.error("更新文件状态失败，文件记录不存在 => fileMd5: {}, fileName: {}", fileMd5, fileName);
                    throw new RuntimeException("文件记录不存在: " + fileMd5);
                }
