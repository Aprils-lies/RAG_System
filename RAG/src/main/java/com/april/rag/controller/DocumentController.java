...
import com.april.rag.service.DocumentService;
import com.april.rag.utils.JwtUtils;
...
/**
 * @Author: 四月是你的谎言
 * @CreateTime: 2026/2/11 21:21
 * Description:文档控制器类，处理文档相关操作请求
 */
...
public class DocumentController {
    ...
    public ResponseEntity<?> deleteDocument(
        ...
        try {
            ...
            // 获取文件信息
            FileUpload file = fileUploadMapper.selectByFileMd5AndUserId(fileMd5, userId);
            if (file == null) {
    ...
    public ResponseEntity<?> downloadFileByName(
        ...
        try {
            ...
            if (userId == null) {
                // 查找公开文件
                FileUpload file = fileUploadMapper.selectPublicByFileName(fileName);
                if (file == null) {
                    Map<String, Object> response = new HashMap<>();
                    response.put("code", HttpStatus.NOT_FOUND.value());
    ...
    private String getOrgTagName(String tagId) {
        ...
        try {
            OrganizationTag tagOpt = organizationTagMapper.findByTagId(tagId);
            if (tagOpt != null) {
                return tagOpt.getName();
            } else {
